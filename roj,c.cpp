...
void addItem() {
    ...
    int count = 0, i, j;  // Declare 'j' here
    FILE *fp = fopen("canteen.txt", "r");

    if (fp != NULL) {
        while (fscanf(fp, "%d %[^\n]s %f", &items[count].id, items[count].name, &items[count].price) != EOF) {
            count++;
        }
        fclose(fp);
    }
    ...
    scanf(" %[^\n]s", item.name);  // To read the item name with spaces
    ...
    // Sort items by ID
    for (i = 0; i < count - 1; i++) {
        for (j = i + 1; j < count; j++) {  // 'j' is now declared and used correctly
...
void viewItems() {
    ...
    printf("\nID\tName\tPrice\n");
    printf("----------------------------\n");
    while (fscanf(fp, "%d %[^\n]s %f", &item.id, item.name, &item.price) != EOF) {
...
void deleteItem() {
    ...
    // Delete the item by shifting others
    for (i = 0; i < count; i++) {
...
void placeOrder() {
    ...
    while (orderId != 0) {
        ...
        int found = 0;
        rewind(fp);  // Ensure the file pointer is reset before reading again
        while (fscanf(fp, "%d %[^\n]s %f", &item.id, item.name, &item.price) != EOF) {
            if (item.id == orderId) {
                found = 1;
